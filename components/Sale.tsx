...
export default function Sale() {
  ...
  const [isClient, setIsClient] = useState(false); // Check if component is running on the client

  useEffect(() => {
    setIsClient(true); // Once mounted, set to true
  }, []);

  useEffect(() => {
    if (isClient) {
      const popupStatus = sessionStorage.getItem("popup") !== "false";
      setIsPopupShown(popupStatus);
  ...
  // Ensure this part only renders if we are on the client
